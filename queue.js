...
addToQueue(object, firstElementExecute = true, notExecButSkip = false) {return new Promise((resolve, reject) => {let funcType = false;if(object.afterEvent) {if(!object.afterEvent.emitter || !object.afterEvent.event) return reject("Missing emitter or event options in afterEvent");}
...
_executeQueue(resolve, reject) {if(this.queue.length == 0) return;if(this.paused) return;
else {
let queueF = this.queue[0];
let realResolve = resolve || queueF.resolve;
let realReject = reject || queueF.reject;
if(queueF.functionType) {
try {let result = queueF.value(...queueF.args);
...
this.queue.shift();
this._executeQueue();}, 5);}}
pause() {this.paused = true;}
resume() {this.paused = false;this._executeQueue();}
unQueue(ind = 0) {return new Promise((resolve, reject) => {if (!this.queue[ind]) return reject("Nothing found with this index");this.queue.splice(ind, 1);});}
get allQueueArgs() {
let array = [];
